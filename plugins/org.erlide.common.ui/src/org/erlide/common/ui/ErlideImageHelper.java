...
public class ErlideImageHelper implements IImageHelper,
     ...
     */
    public void dispose() {
        for (final Iterator<Image> iter = registry.values().iterator(); iter
                .hasNext();) {
    ...
    @Inject
    public void hookBundleListener(final AbstractUIPlugin aplugin) {
    ...
    public Image getImage(final String imageName) {
        ...
        final String imgname = imageName == null ? defaultImage : imageName;
        if (imgname != null) {
            Image result = null;
            final URL imgUrl = getPlugin().getBundle().getEntry(
    ...
    public void setPlugin(final AbstractUIPlugin plugin) {
        this.plugin = plugin;
    }
