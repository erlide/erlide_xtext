...
import org.erlide.project.buildpath.util.CharOperation;

/**
 * Definition of a set of access rules used to flag forbidden references to non
 * API code.
 */
public class AccessRuleSet {
     ...
     * 
     * @param accessRules
     *            the access rules to be contained by the new set
     * @param messageTemplates
     *            a Sting[4] array specifying the messages for type,
     *            constructor, method and field access violation; each should
     *            contain as many placeholders as expected by the respective
     *            access violation message (that is, one for type and
     *            constructor, two for method and field); replaced by a default
     *            value if null.
     */
    public AccessRuleSet(final AccessRule[] accessRules,
            final String[] messageTemplates) {
        this.accessRules = accessRules;
        if (messageTemplates != null
                && messageTemplates.length == MESSAGE_TEMPLATES_LENGTH) {
            this.messageTemplates = messageTemplates;
        } else {
            this.messageTemplates = new String[] {
                    "{0}", "{0}", "{0} {1}", "{0} {1}" }; //$NON-NLS-1$ //$NON-NLS-2$//$NON-NLS-3$ //$NON-NLS-4$
        }
    }
    ...
    public boolean equals(final Object object) {
        ...
        for (int i = 0; i < MESSAGE_TEMPLATES_LENGTH; i++) {
            if (!this.messageTemplates[i]
                    .equals(otherRuleSet.messageTemplates[i])) {
                return false;
            }
        }
     ...
     * 
     * @param targetTypeFilePath
     *            the target type file path, formed as:
     *            "org/eclipse/dltk/core/DLTKCore"
     * @return the first access restriction that applies if any, null else
     */
    public AccessRestriction getViolatedRestriction(
            final char[] targetTypeFilePath) {
        for (int i = 0, length = this.accessRules.length; i < length; i++) {
            ...
            if (CharOperation.pathMatch(accessRule.pattern, targetTypeFilePath,
                ...
                case DiscouragedReference:
                    return new AccessRestriction(accessRule,
